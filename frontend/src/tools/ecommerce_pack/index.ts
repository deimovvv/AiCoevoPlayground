...
export const SHOT_CATALOG: Record<string, { label: string; onModel: boolean; framing: string }> = {
  ...
  flat_front:   { label: "Flat · Frente",       onModel: false, framing: "Product-only PACKSHOT: the garment laid FLAT and shot from directly above, FRONT side up, centered. The garment is laid FLAT on the surface and photographed straight from ABOVE (top-down flat-lay). It is COMPLETELY EMPTY and UNWORN: no body, no torso, no bust, no shoulders filled out, no invisible mannequin and NO ghost-mannequin effect. The fabric lies relaxed and slightly soft with natural creases where it settles. Lay out whatever parts the garment actually has, flat and symmetrical: if it is a TOP, the sleeves lie flat alongside the body and the collar lies flat; if it is a BOTTOM (jeans, trousers, shorts, skirt), the two legs lie flat and parallel, side by side, with both hems fully visible, complete and SHARP. It must read as clothing resting on a surface — NOT as a garment standing up, holding a 3D body shape or being worn. EVERY part of the garment is in SHARP FOCUS from edge to edge, evenly lit and fully rendered: no blur, no motion blur, no soft or smeared areas, no fading, no dissolving into the background, no melted, warped or unfinished edges — especially at the hems, cuffs and corners. The garment ends in clean, crisp, well-defined edges against the backdrop.  The ENTIRE garment must fit INSIDE the frame with comfortable margin: every sleeve, hem, collar and corner fully visible, nothing touching, overflowing or cropped by any edge. Leave roughly 10% empty background on all four sides. If a sleeve or hem extends diagonally, ZOOM OUT until the whole garment fits — never crop it. NO person, NO model, NO body — only the garment." },
  flat_back:    { label: "Flat · Espalda",      onModel: false, framing: "Product-only PACKSHOT of the garment laid FLAT and shot from directly above, BACK side up, centered. The garment is laid FLAT on the surface and photographed straight from ABOVE (top-down flat-lay). It is COMPLETELY EMPTY and UNWORN: no body, no torso, no bust, no shoulders filled out, no invisible mannequin and NO ghost-mannequin effect. The fabric lies relaxed and slightly soft with natural creases where it settles. Lay out whatever parts the garment actually has, flat and symmetrical: if it is a TOP, the sleeves lie flat alongside the body and the collar lies flat; if it is a BOTTOM (jeans, trousers, shorts, skirt), the two legs lie flat and parallel, side by side, with both hems fully visible, complete and SHARP. It must read as clothing resting on a surface — NOT as a garment standing up, holding a 3D body shape or being worn. EVERY part of the garment is in SHARP FOCUS from edge to edge, evenly lit and fully rendered: no blur, no motion blur, no soft or smeared areas, no fading, no dissolving into the background, no melted, warped or unfinished edges — especially at the hems, cuffs and corners. The garment ends in clean, crisp, well-defined edges against the backdrop.  The ENTIRE garment must fit INSIDE the frame with comfortable margin: every sleeve, hem, collar and corner fully visible, nothing touching, overflowing or cropped by any edge. Leave roughly 10% empty background on all four sides. If a sleeve or hem extends diagonally, ZOOM OUT until the whole garment fits — never crop it. NO person, NO model — only the garment." },
...
const FLAT_NO_SHADOW = "The garment is presented as a clean FLAT-LAY packshot lying flat on the surface, completely empty and unworn (NO ghost mannequin, NO invisible body, NO filled-out volume) — it is NOT a standing figure, so it casts NO grounding shadow, NO drop shadow and NO projected shadow. Keep the background perfectly clean, uniform and even, with no shadow, no contact-shadow gradient and no dark falloff anywhere around the garment.";
...
const handleGenerate: StepHandler = async (ctx) => {
  ...
  // Detalles a respetar (texto libre del usuario) — se AGREGA al prompt de TODAS las tomas
  // (on-model y flats) como alta prioridad. Para forzar detalles que el modelo suele limpiar
  // (botón, largo, puños, tapeta…). El usuario nombra prenda + detalle.
  const detailsText = ((cfg.ecomDetails as string) || "").trim();
  const detailsClause = detailsText
...
Output: the person from image 1, EXACTLY as they appear in image 1 (same skin, same jewelry, same clothing, same accessories, same face), re-posed to match the body geometry of image 2. The face must stay perfectly recognizable as the person in image 1 — do NOT let image 2's face leak in. ${BODY_PROPORTIONS} ${FACE_REALISM} ${PIXEL_FIDELITY} ${REALISM_NEGATIVES}${NO_TEXT}`;
    ...
    if (i === 0 || !anchorUrl) {
      ...
      if (shotPoseUrl) {
        ...
        desc.push(`Image ${idx}: POSE REFERENCE — copy ONLY the body posture, body orientation and camera framing${curSP.pose ? ` (${curSP.pose})` : ""}. EVERYTHING ELSE in this image is a DECOY and must be COMPLETELY DISCARDED — its clothing AND their colours${curSP.ignore ? ` (specifically: ${curSP.ignore})` : ""}, its face, hair, skin, identity, accessories, shoes, jewelry, props, AND its background, scene, floor and lighting must NOT appear in the output. Take ONLY the body posture, orientation and framing from this image — NOTHING else. The person's identity comes ONLY from the IDENTITY image; the garments and their colours come ONLY from the GARMENT reference(s); the scene comes ONLY from the prompt.`);
        idx++;
      }
...
- STRICTLY IGNORE everything else in the POSE reference — its clothing AND their colours, hair, face, skin, accessories, shoes, jewelry, props/objects held, and its background, scene, floor and lighting are ALL a DECOY and must NOT appear in the output.${poseCur.ignore ? ` Specifically discard: ${poseCur.ignore}.` : ""} The garment the model wears (and its colour) comes ONLY from the GARMENT reference (the real product), never from the pose reference person; the scene comes only from the prompt.
- ${IDENTITY_LOCK}
- ${FACE_REALISM}
- ${GARMENT_ORIENTATION}
...
- ${BODY_PROPORTIONS}`
  ...
  await Promise.all(flatShots.flatMap((sid) => {
    ...
    return flatSubjects.map(async (subj) => {
      ...
      const prompt = `Professional e-commerce product packshot of a single garment. ${studioClauseFlat} ${shot.framing} Show ONLY this one garment — no other clothing items. ${cameraLighting} ${FABRIC_REALISM} ${PIXEL_FIDELITY} ${REALISM_NEGATIVES}${detailsClause}${NO_TEXT}\n\nREFERENCE IMAGES:\n${desc.join("\n")}`;
      try {
